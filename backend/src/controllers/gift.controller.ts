    ...
    try {
        ...
        if (interests) {
            ...
            if (interestList.length > 0) {
                where.OR = [
                    { tags: { contains: interestList[0] } },
                    { description: { contains: interestList[0] } },
                    { name: { contains: interestList[0] } }
                ];
            }
        }
...
export const createGift = async (req: Request, res: Response) => {
    try {
        const { name, description, basePrice, customPrice, category, tags, imageUrl, vendorId } = req.body;
        ...
        const gift = await prisma.gift.create({
            data: {
                ...
                customPrice: parseFloat(customPrice || '15'),
                category,
